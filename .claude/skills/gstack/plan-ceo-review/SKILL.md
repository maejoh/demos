...
allowed-tools:
  ...
  - AskUserQuestion
---
<!-- Forked from garrytan/gstack plan-ceo-review v1.0.0 — update check removed (standalone fork) -->
...
```
Then read CLAUDE.md and any existing architecture docs. When reading TODOS.md (if it exists), specifically:
...
* Rollback posture. If this ships and immediately breaks, what's the rollback procedure? Git revert? Feature flag? How long?
...
```
  ...
  -------------------------|-----------------------------|-----------------
  ExampleService#call      | API timeout                 | NetworkError
                           | API returns 429             | RateLimitError
                           | API returns malformed JSON  | SyntaxError
                           | DB connection failure       | ConnectionError
                           | Record not found            | NotFoundError
  -------------------------|-----------------------------|-----------------

  EXCEPTION CLASS      | CAUGHT?   | CATCH ACTION           | USER SEES
  ---------------------|-----------|------------------------|------------------
  NetworkError         | Y         | Retry 2x, then throw   | "Service unavailable"
  RateLimitError       | Y         | Backoff + retry         | Nothing (transparent)
  SyntaxError          | N ← GAP   | —                      | 500 error ← BAD
  ConnectionError      | N ← GAP   | —                      | 500 error ← BAD
  NotFoundError        | Y         | Return null, log warn   | "Not found" message
```
Rules for this section:
* Catching generic `Error` is ALWAYS a smell. Name the specific error type.
...
* For LLM/AI service calls specifically: what happens when the response is malformed? When it's empty? When it hallucinates invalid output? Each of these is a distinct failure mode.
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.

### Section 3: Security & Threat Model
Security is not a sub-bullet of architecture. It gets its own section.
Evaluate:
* Attack surface expansion. What new attack vectors does this plan introduce? New endpoints, new params, new file paths?
* Input validation. For every new user input: is it validated, sanitized, and rejected loudly on failure? What happens with: null, empty string, wrong type, oversized input, injection attempts?
...
* Secrets and credentials. New secrets? In env vars, not hardcoded? Documented in CLAUDE.md?
...
* What is the edge case test? (null, empty, boundary values)
...
Evaluate:
* Data fetching patterns. For every new data fetch: is it batched? Paginated? Could it return unbounded results?
* Memory usage. For every new data structure: what's the maximum size in production?
* Caching opportunities. For every expensive computation or external call: should it be cached?
* Slow paths. Top 3 slowest new codepaths and estimated latency.
* Bundle size impact. New npm packages or large imports affecting client-side load time?
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.

### Section 8: Observability & Debuggability Review
New systems break. This section ensures you can see why.
Evaluate:
* Logging. For every new codepath: is there enough logging to reconstruct what happened from logs alone 3 weeks later?
* Error visibility. Are errors surfaced to the user clearly, or do they fail silently?
* Debuggability. If a bug is reported post-deploy, can you reproduce it locally?
* Environment parity. Will this work the same in development, preview, and production? (Especially: Redis namespacing, env vars, Vercel edge cases.)
...
Evaluate:
* Vercel deployment safety. Does this deploy cleanly via git push → Vercel auto-deploy?
* Environment variables. Any new env vars? Documented in CLAUDE.md? Added to Vercel dashboard?
* Preview vs. production parity. Will the preview deployment work correctly (Redis namespace, etc.)?
* Rollback plan. If this ships and immediately breaks, what's the rollback? Git revert? How fast?
* Post-deploy verification. First 5 minutes: what do you check?

**EXPANSION mode addition:**
* What deploy infrastructure would make shipping this demo routine?
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.

### Section 10: Long-Term Trajectory Review
Evaluate:
* Technical debt introduced. Code debt, operational debt, documentation debt.
* Path dependency. Does this make future demos or features harder?
* Portfolio signal. Will this impress an engineer reviewing the repo? Is it self-explanatory?
* Reversibility. Rate 1-5: 1 = one-way door, 5 = easily reversible.
* The 1-year question. Read this plan as a new engineer in 12 months — obvious?

**EXPANSION mode additions:**
* What comes after this ships? Does the architecture support that trajectory?
* Platform potential. Does this create capabilities other demos can leverage?
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
Complete table of every method that can fail, every error type, caught status, catch action, user impact.
...
### TODOS.md updates
If TODOS.md exists: present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. If TODOS.md doesn't exist, skip silently.

### Delight Opportunities (EXPANSION mode only)
Identify at least 5 "bonus chunk" opportunities (<30 min each) that would make users think "oh nice, they thought of that." Present each as its own AskUserQuestion.
...
```
  ...
  | TODOS.md updates     | ___ items proposed (or skipped — no file)   |
