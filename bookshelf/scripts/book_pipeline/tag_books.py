...
def build_assignment_prompt(books: dict, vocabulary: list[str]) -> str:
    ...
    return (
        ...
        "Rules:\n"
        "- Assign 2-5 tags per book\n"
        "- Only use tags from the vocabulary above — never invent new ones\n"
        "- Always include specific tags (e.g. 'Python', 'MCP') when the title or description "
        "makes them explicit — do not drop them in favour of broad categories\n"
        "- Include broad category tags (e.g. 'Machine Learning') in addition to specific ones, "
        "not instead of them\n\n"
        "Example output:\n"
