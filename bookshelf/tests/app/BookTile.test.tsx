...
describe("BookTile", () => {
  describe("BookHeader", () => {
    it("renders the book title and author", () => {
      render(<BookTile book={makeBook()} votes={0} onVote={vi.fn()} />)
      expect(screen.getByText("Test Book")).toBeInTheDocument()
      expect(screen.getByText("Some Author")).toBeInTheDocument()
    })

    it("renders topic tags as chips", () => {
      render(<BookTile book={makeBook({ tags: ["Python", "Databases"] })} votes={0} onVote={vi.fn()} />)
      expect(screen.getByText("Python")).toBeInTheDocument()
      expect(screen.getByText("Databases")).toBeInTheDocument()
    })

    it("renders AI tags when present", () => {
      render(<BookTile book={makeBook({ ai_tags: ["Machine Learning"] })} votes={0} onVote={vi.fn()} />)
      expect(screen.getByText("Machine Learning")).toBeInTheDocument()
    })

    it("renders no AI tag chips when ai_tags is absent", () => {
      const { container } = render(<BookTile book={makeBook({ ai_tags: undefined })} votes={0} onVote={vi.fn()} />)
  ...
  describe("BookCover", () => {
    it("renders a cover image when coverUrl is present", () => {
      render(<BookTile book={makeBook({ title: "Covered", coverUrl: "/covers/test.jpg" })} votes={0} onVote={vi.fn()} />)
      expect(screen.getByAltText("Cover of Covered")).toBeInTheDocument()
    })

    it("renders a placeholder div when coverUrl is absent", () => {
      render(<BookTile book={makeBook({ coverUrl: undefined })} votes={0} onVote={vi.fn()} />)
  ...
  describe("VoteButton", () => {
    it("displays the current vote count", () => {
      render(<BookTile book={makeBook()} votes={17} onVote={vi.fn()} />)
